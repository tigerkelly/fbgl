...
typedef struct _fbgl {
	...
	char *dbFont[MAX_FONT_NUMBER];
	Color *dbColor[MAX_COLOR_NUMBER];
	FT_Library library;
	FT_Face defaultFace;
...
FbglFont *fbglNewFont(char *fontName, int fontSize);
...
int fbglLoadFonts(FBGL *fbgl, FbglFont *fbglFont, FT_Face face);
...
// Create widgets
FbglWidget *fbglNewText(FBGL *fbgl, char *name, char *text, int x, int y, char *style);
